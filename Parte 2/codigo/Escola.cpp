 ...
 * @param m string with the school's address
 * @param m string with the school principal's name
 * */
Escola::Escola(string n, string z, string m, string d) : nome(n), zona(z), morada(m), diretor(d)
{
	num_utentes = 0;
}
...
* */
string Escola::getMorada() const
{
	return morada;
}

/*!
* @return the school principal's name
* */
string Escola::getDiretor() const
{
	return diretor;
}

/*!
* @return the number of company clients that study/work in this school
* */
int Escola::getNumUtentes() const
{
	return num_utentes;
}

/*!
* Increases the number of company clients that study/work in this school
* */
void Escola::increaseNumUtentes()
{
	num_utentes++;
}

/*!
* Decreases the number of company clients that study/work in this school
* */
void Escola::decreaseNumUtentes()
{
	num_utentes--;
}
 ...
 * */
bool Escola::operator==(const Escola &esc2) const
{
	return (nome == esc2.getNome());
}

/*!
* Defines the operator < for class Escola as a comparison between the number of clients and the names of the schools.
* @param esc2 object reference of class Escola
* */
bool Escola::operator<(const Escola &esc2) const
{
	if (num_utentes == esc2.getNumUtentes())
		return (nome < esc2.getNome());
	return (num_utentes < esc2.getNumUtentes());
}

/*!
 * Defines the operator << for class Escola and outputs the atributes of the school.
 * @param out ostream reference
 * @param e object reference of class Escola
 * */
ostream & operator<<(ostream &out, const Escola& e)
{
	out << e.getNome() << endl << e.getZona() << endl << e.getMorada() << endl << e.getDiretor() << endl;
	return out;
}
