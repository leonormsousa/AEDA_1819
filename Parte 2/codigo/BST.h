...
class BST
{
 public:
  ...
  const Comparable & find_new(const Comparable & x) const;
  bool isEmpty( ) const;
 ...
 private:
  ...
  BinaryNode<Comparable> * find_new(const Comparable & x, BinaryNode<Comparable> *t) const;
...
template <class Comparable>
const Comparable & BST<Comparable>::find_new(const Comparable & x) const
{
	return elementAt(find_new(x, root));
}
...
template <class Comparable>
BinaryNode<Comparable> * BST<Comparable>::find_new(const Comparable & x, BinaryNode<Comparable> *t) const
{
	if (t == NULL)
		return NULL;
	if (x == t->element)
		return t;
	BinaryNode<Comparable> * no = find_new(x, t->left);
	if (no == NULL)
		no = find_new(x, t->right);
	return no;
}
